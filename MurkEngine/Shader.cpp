...
Shader::Shader(const std::string& vertexSource, const std::string& fragmentSource)
{
	...
	if (compileStatus == GL_FALSE) MURK_CRITICAL("Fragment shader error: {0}",  GetShaderInfoLog(fragmentShader));
	glGetShaderiv(vertexShader, GL_COMPILE_STATUS, &compileStatus);
	if (compileStatus == GL_FALSE) MURK_CRITICAL("Vertex shader error: {0}", GetShaderInfoLog(vertexShader));
...
char* Murk::Shader::GetShaderInfoLog(GLint shader)
{
	GLint length;
